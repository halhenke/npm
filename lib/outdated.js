...
function outdated_ (args, dir, parentHas, depth, cb) {
  ...
  readInstalled(path.resolve(dir), { dev : true }, function (er, data) {
    ...
    // console.log(`readInstalled: pkgs is `, data.dependencies);

    // var links = pkgs.filter(function (p) {
    //   return data.dependencies[p].link
    // })
    // pkgs = pkgs.filter(function (p) {
    //   return links.indexOf(p) < 1
    // })

    // links.forEach(function (link) {
    //   // delete pkgs[link.name]
    //   // console.log(`link is ${link.name}`);
    //   delete pkgs[link]
...
function shouldUpdate (args, dir, dep, has, req, depth, cb, type) {
  ...
  // cant rely on npm-parse-argument....
  // if (parsed.type === "link" || (parsed.hosted && parsed.hosted.type === "github")) {
  if (curr.link) {
    return doIt("linked", "linked")
  }

  // fs.lstat(path.resolve(dir, "node_modules", dep), function (er, stat) {
  //   // console.log(`name = ${dep}, stat is link?: `, stat.isSymbolicLink());
